...
export default class Game {
  ...
  setCurrentScreen(screenName = Game.initialMenuScreen) {
    let newScreen = this.#allMenuScreens.find(
  ...
  updateOptionsScreen() {
    ...
    screen.options[0] = this.#state.enableSound
      ? Game.options.DISABLESOUNDOPTION
      : Game.options.ENABLESOUNDOPTION;
  }
  ...
  runPauseScreen(keys) {
    ...
    if (keys["ArrowLeft"] && this.#arrowLeftRelease) {
    ...
    if (keys["ArrowRight"] && this.#arrowRightRelease) {
      let index = currentScreen.options.indexOf(selection);
      if (index === 0) {
        this.#currentMenuSelection = currentScreen.options[1];
      } else {
  ...
  populateLevelSelect(levelNames) {
    const levelSelect = this.#allMenuScreens.find(
      (screen) => screen.type === Game.screenTypes.LEVEL,
    );
    const returnOption = ["Return to", "Title Screen"];
  ...
  handleMovement(deltaTime, keys) {
    let newMove = new Move(this.#currentLevel, this.#player, keys, deltaTime);
    ...
    if (
      ...
      if (currentScreen.type === Game.screenTypes.OPTIONS) {
        if (!this.#state.controlVolumeSlider) {
          while (
            currentScreen.options[index] === Game.options.CONFIRMVOLUMEOPTION ||
    ...
    if ((keys["z"] || keys["Z"]) && this.#zRelease) {
      if (currentScreen.type !== Game.screenTypes.SOUND) {
        if (selection === Game.options.ENABLESOUNDOPTION) {
          ...
          selection === Game.options.ADJUSTVOLUME ||
          ...
          if (this.#state.controlVolumeSlider) {
            ...
            const sliderIndex = currentScreen.options.indexOf(
              Game.options.SLIDER,
            );
            this.#currentMenuSelection = currentScreen.options[sliderIndex];
          }
        } else if (
          selection === Game.options.CONFIRMVOLUMEOPTION &&
  ...
  handleScreenChange() {
    ...
    if (screen.name !== Game.screenNames.LEVEL || Array.isArray(selection)) {
      if (selection === Game.options.TUTORIAL) {
        ...
        this.setCurrentScreen(
          `` + `${screen.name.slice(0, -1)}` + `${parseInt(pageNum, 10) + 1}`,
        );
        ...
        selection === Game.options.PLAYSOUND ||
