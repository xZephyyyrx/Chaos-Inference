...
export default class Move {
  ...
  handleInput() {
    if (Move.activeWalljumpFrames > 0) {
      ...
      if (this.keys["z"] || (this.keys["Z"] && Move.zKeyRelease)) {
        ...
        if (Move.hasTriggeredWallStick && Move.zKeyRelease) {
        ...
        // Standard Jump
        if (
          (this.collisions.down && !Move.hasJumped) ||
      ...
      if (
        (!this.keys["z"] && !this.keys["Z"] && this.velY < 0) ||
  ...
  handleWalljumps() {
    if (
      ...
      !this.jumpCollidesWithWall() &&
      Move.activeWalljumpBufferFrames <= 0
    ) {
      Move.hasTriggeredWallStick = false;
    } else if (
      !this.jumpCollidesWithWall() &&
  ...
  jumpCollidesWithWall() {
    return (
      !this.collisions.down && (this.collisions.left || this.collisions.right)
    );
  }
